pfTransform2 <-
  function(ID = NULL,
        ...
        if (is.null(MethodType)) {
          ...
          for (i in ID) {
            paleofiredata[paleofiredata$ID_SITE %in% i &
                            !(paleofiredata$UNIT %in% paleofiresites$pref_units[paleofiresites$id_site == i]), 7] <-
              NA
          }
          ...
          if (QuantType == "INFL") {
            for (i in ID)
              if (!(unique(paleofiredata[paleofiredata[, 1] == i, 7]) %in% "INFL") &
                  ...
                  sum(paleofiredata[paleofiredata[, 1] == i, 2]) > 0) {
                # Mod 28 02 2018 avoid influx calculus when depth not in database (occur for v 4.0.0)
                infl <-
                  influx(paleofiredata[paleofiredata[, 1] == i, ])
          ...
          paleofiredata <-
            paleofiredata[paleofiredata[, 6] %in% MethodType, ]
          for (i in ID) {
            if (length(unique(paleofiredata[paleofiredata[, 1] %in% i, 5])) >= 2) {
              paleofiredata[paleofiredata[, 1] %in% i &
                              !(paleofiredata[, 5] %in% paleofiresites$pref_units[paleofiresites[, 1] == i]), 7] <-
                NA
            }
          }
          paleofiredata <- paleofiredata[!is.na(paleofiredata$TYPE), ]
    ...
    if (is.list(ID) & length(ID) > 2) {
      ...
      ids <-
        matrix(nrow = length(temp[, 1]), ncol = length(temp[1, ]))
    ...
    if (Interpolate == TRUE) {
      ...
      for (k in 1:length(ID)) {
        if (length(paleofiredata[paleofiredata[, 1] == ID[k], 3]) >= 3) {
          rawI[, k] <-
            approx(paleofiredata[paleofiredata[, 1] == ID[k], 3], paleofiredata[paleofiredata[, 1] == ID[k], 4], AgeN, method = "linear")$y
        } else {
          print(paste(
            IDChar$site_name[k],
            "has < 3 charcoal values and was excluded",
            sep = " "
          ))
        }
      }
      ...
      for (k in 1:length(ID)) {
        if (is.na(sum(paleofiredata[paleofiredata[, 1] == ID[k], 2])) == F) {
          if (length(paleofiredata[paleofiredata[, 1] == ID[k], 3]) >= 3) {
            depthI[, k] <-
              approx(paleofiredata[paleofiredata[, 1] == ID[k], 3], paleofiredata[paleofiredata[, 1] == ID[k], 2], AgeN,
                     method = "linear")$y
          }
        } else {
          depthI[, k] <- NA
    ...
    if (Interpolate == FALSE) {
      ...
      for (k in 1:length(ID)) {
        lengths[k] <-
          c(length(paleofiredata[paleofiredata[, 1] %in% ID[k], 1]))
      }
      m <- max(lengths)
      ...
      for (k in 1:length(ID)) {
        ...
        AgeTemp <-
          c(paleofiredata[paleofiredata[, 1] %in% ID[k], 3], rep(NA, forNA))
        Ages[, k] <- c(AgeTemp)
        rawTemp <-
          c(paleofiredata[paleofiredata[, 1] %in% ID[k], 4], rep(NA, forNA))
        rawI[, k] <- c(rawTemp)
        depthTemp <-
          c(paleofiredata[paleofiredata[, 1] %in% ID[k], 2], rep(NA, forNA))
      ...
      for (j in 1:length(method)) {
        ...
        for (k in 1:length(ID)) {
          ...
          if (length(tmp[, 1]) > 3 & ID[k] != 882) {
            ...
            if (methodj == "Z-Score") {
              mu <-
                mean(tmp[tmp[, 1] >= BasePeriod[1] &
                           tmp[, 1] <= BasePeriod[2], 2])
              sigma <-
                sd(tmp[tmp[, 1] >= BasePeriod[1] &
                         tmp[, 1] <= BasePeriod[2], 2])
              ...
              # Z-Score otherwise
                ...
                transI[, k] <-
                  approx(tmp[, 1], (tmp[, 2] - mu) / sigma, Ages[, k])$y
              }
            }
            if (methodj == "Box-Cox") {
              transI[, k] <-
                approx(tmp[, 1],
                       pfBoxCox(tmp[, 2], alpha = alpha, type = type),
                       Ages[, k])$y
            }
            if (methodj == "LOESS") {
              transI[, k] <-
                approx(tmp[, 1], predict(loess(tmp[, 2] ~ tmp[, 1], span = span)), Ages[, k])$y
            }
            if (methodj == "MinMax") {
            ...
            if (methodj == "RunMed") {
              w <-
                round(RunWidth / ((max(tmp[, 1]) - min(tmp[, 1])) / length(tmp[, 1])))
              if (gtools::odd(w))
                w <- w
              else
                w <- w + 1
              transI[, k] <-
                approx(tmp[, 1], runmed(tmp[, 2], w), Ages[, k])$y
            }
            if (methodj == "RunMean") {
              w <-
                round(RunWidth / ((max(tmp[, 1]) - min(tmp[, 1])) / length(tmp[, 1])))
              if (gtools::odd(w))
                ...
                w <- w + 1
              transI[, k] <-
                approx(tmp[, 1], caTools::runmean(tmp[, 2], w), Ages[, k])$y
            }
            if (methodj == "RunMin") {
              w <-
                round(RunWidth / ((max(tmp[, 1]) - min(tmp[, 1])) / length(tmp[, 1])))
              if (gtools::odd(w))
                ...
                w <- w + 1
              transI[, k] <-
                approx(tmp[, 1], caTools::runmin(tmp[, 2], w), Ages[, k])$y
            }
            if (methodj == "RunMax") {
              w <-
                round(RunWidth / ((max(tmp[, 1]) - min(tmp[, 1])) / length(tmp[, 1])))
              if (gtools::odd(w))
                ...
                w <- w + 1
              transI[, k] <-
                approx(tmp[, 1], caTools::runmax(tmp[, 2], w), Ages[, k])$y
            }
            if (methodj == "RunQuantile") {
              ...
              transI[, k] <-
                approx(tmp[, 1],
                       caTools::runquantile(tmp[, 2], w, RunQParam),
                       Ages[, k])$y
            }
            if (methodj == "SmoothSpline") {
              transI[, k] <-
                approx(tmp[, 1],
                       smooth.spline(tmp[, 1], tmp[, 2], spar = span)$y,
                       Ages[, k])$y
            }
            #         if (methodj=="GAM"){
            ...
            if (methodj == "Hurdle") {
              ...
              transI[, k] <-
                approx(tmp[, 1],
                       pscl::hurdle(tmp[, 2] ~ tmp[, 1])$fitted.values,
                       Ages[, k])$y
            }
          }
          Sys.sleep(0.00002)
