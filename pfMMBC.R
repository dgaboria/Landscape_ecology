...
pfMinMax <- function(serie) {
  serie <- (serie - min(serie, na.rm = TRUE)) / 
    (max(serie, na.rm = TRUE) - min(serie, na.rm = TRUE))
  return(serie)
}
...
pfBoxCox <-
  function(serie, alpha = 0.01, type = "BoxCox1964") {
    ...
    if (alpha == "alternative") {
      ...
      # alternative alpha: 0.5 time the smallest nonzero value of quant
    }
    quant2 <- serie + alpha
    
    # maximum likelihood estimation of lambda
    # derived from the boxcox.R function in the Venables and Ripley 
    ...
    # Box-Cox original
    if (type == "BoxCox1964") {
      if (lafit == 0.0) tquant <- log(quant2) 
